...
#########################################################################
########## For reading and adjusting labels on variables #################
##########################################################################

adjust_label_pre=function(labstr){
  #removing weird lead-ins like Demo: de023 befor ':'
...
## After sorting variables in categorical, index, binary, discrete, continuous, etc.
## We again adjust labels to shorten thier length
adjust_label_post=function(labstr){
  labstr=gsub("2 weeks (el) or 4 weeks (bl)","2 or 4 weeks",labstr) #check short hand
  labstr=gsub(" ( [^!]*)","",labstr) #remove text in parentheses
  labstr=gsub("[^!]*:","",labstr) #remove text followed by :
  labstr=gsub("Indicator for ","",labstr) #remove 'Indicator for' lead in
  labstr=gsub("Hours per week","Weekly hours",labstr,ignore.case = T) #shorten phrasing
  labstr=gsub("\\([^!]*","",labstr)
  labstr=gsub("add risk averse","",labstr) #remove extra phrase
  labstr=gsub("-- [^!]*","",labstr) #remove anything after '--'
  if(length(labstr)>1){
...
## Manual adjustments based on Blattman et al. (2016) in-text explanations
manual_adjust=function(df.range,nstddev,liberia.sub){
  ...
  #man19=c("riskdecl_b","Declared risk appetite","Continuous",as.character(-nstddev),as.character(nstddev))
...
## Find index variables from labels
...
## find binary variables from labels and nVals
...
## (variable name, label, variable type (e.g. Binary), min & max observed values,
##    indicators for in subset or reduced subset of covariates, role in model (e.g. repsonse),
##    # of values observed, # of na observations)
variable_info=function(labstr,liberia.sub,sub.covariates,red.sub.covariates,response.vars,block.vars,treat.vars){
  ...
  df.range=manual_adjust(df.range = df.range,liberia.sub=liberia.sub)
  df.range=df.range[,c("variable","label","type","obs.min","obs.max","subset","reduced.subset","model.role","nVals","nNa")]
  return(df.range)
}


variable_label_tables=function(basepath=rprojroot::find_rstudio_root_file(),
                               outputpath=paste0(rprojroot::find_rstudio_root_file(),"/output"),
                               datainpath=paste0(rprojroot::find_rstudio_root_file(),"/data/liberia_replication_data/LiberiaRound5.Rda")){
  ...
  cat.vars.df=var.df[(!is.na(var.df$type))&(var.df$type=="Binary"|var.df$type=="Categorical"),]
  index.vars.df=var.df[(!is.na(var.df$type))&(var.df$type=="Index"),]
  ...
  # # #Change the path below to the STYL_Final.dta file from Blattman et al (2020) replication files download
  ...
  if(length(missing)>0){
    ...
    miss.vars=base::merge(miss.vars,var.df[var.df$variable %in% missing,c("variable","nVals")],by="variable",sort=F)
    rownames(miss.vars)=miss.vars$variable
